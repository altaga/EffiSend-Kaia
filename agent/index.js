...
const transferTokens = tool(
  async ({ amount, to, token }, { configurable: { user } }) => {
    ...
    const { hash } = response;
    return JSON.stringify({
      message: "Transaction created and available on Kaia Mainnet.",
      status: "success",
      transaction: hash,
    });
  },
...
const swapTokens = tool(
  async ({ amount, fromToken, toToken }, { configurable: { user } }) => {
    ...
    return JSON.stringify({
      status: "success",
      message: "Your transaction is available on Kaia Mainnet.",
      transaction: hash,
    });
  },
  {
...
// Transfer Kaia USDT to Arbitrum USDT (KAST Card) - Modified to return transaction data to API
...
const listOfTools = tool(
  () => {
    ...
    return JSON.stringify({
      status: "info",
      message:
        "DeSmond can help you fund your Kast card, transfer tokens on Kaia Mainnet, Swap tokens on Kaia Mainnet, and retrieve your current balance for any token on Kaia Mainnet.",
    });
  },
  {
    name: "list_of_tools",
